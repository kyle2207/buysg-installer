...
# PowerShell-first shim: PowerShell resolves buysg.ps1 BEFORE buysg.cmd, so for
# PowerShell users (Windows default shell) Ctrl+C exits cleanly instead of
# leaving cmd.exe's "Terminate batch job (Y/N)?" prompt. cmd.exe ignores .ps1
# and keeps using buysg.cmd.
# Guard: under a Restricted/AllSigned policy the .ps1 would BREAK the command
# (resolution still prefers it, execution then fails), so only ship it when
# scripts can actually run - and drop any stale copy otherwise. Effective
# policy is computed WITHOUT the Process scope: "buysg update" relaunches this
# installer with -ExecutionPolicy Bypass, which must not fool the check.
$PsShimPath = Join-Path $Bin 'buysg.ps1'
$Ep = 'Undefined'
foreach ($scope in 'MachinePolicy', 'UserPolicy', 'CurrentUser', 'LocalMachine') {
    $p = Get-ExecutionPolicy -Scope $scope
    if ($p -ne 'Undefined') { $Ep = $p; break }
}
if ($Ep -eq 'Undefined') { $Ep = 'Restricted' }   # all Undefined = client default
if ($Ep -in @('Restricted', 'AllSigned')) {
    if (Test-Path $PsShimPath) { Remove-Item -Force $PsShimPath }
    Write-Host "    (execution policy is $Ep - PowerShell shim skipped, buysg.cmd is used;"
    Write-Host "     to get clean Ctrl+C behavior in PowerShell, run once:"
    Write-Host "       Set-ExecutionPolicy -Scope CurrentUser RemoteSigned"
    Write-Host "     and then reinstall / 'buysg update')"
} else {
    $shimPs = @'
# buysg PowerShell shim (auto-generated by install.ps1; buysg.cmd next to this
# file is the cmd.exe fallback). PowerShell resolves .ps1 first, so Ctrl+C
# exits cleanly without cmd.exe's "Terminate batch job (Y/N)?" prompt.
$Root = Join-Path $env:LOCALAPPDATA 'buysg'
$Exe  = Join-Path $Root 'venv\Scripts\buysg.exe'
$env:BUYSG_HOME = Join-Path $Root 'home'
...
if ($Cmd -eq 'update') {
    ...
    Write-Host 'Updating buysg to the latest release...'
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "irm https://raw.githubusercontent.com/kyle2207/buysg-installer/main/install.ps1 | iex"
    exit $LASTEXITCODE
}

if ($Cmd -eq 'uninstall') {
    Write-Host "This removes the whole folder: $Root"
    Write-Host 'NOTE: home\config\config.ini and home\certificates\ are deleted too - back them up first!'
    $confirm = Read-Host 'Remove? (y/n)'
    if ($confirm -ne 'y') { Write-Host 'Cancelled.'; exit 0 }
    $b = Join-Path $Root 'bin'
    $p = [Environment]::GetEnvironmentVariable('Path', 'User')
    $n = (($p -split ';') | Where-Object { $_ -and $_ -ne $b }) -join ';'
    [Environment]::SetEnvironmentVariable('Path', $n, 'User')
    Start-Process -WindowStyle Minimized cmd.exe -ArgumentList "/c timeout /t 2 >nul & rmdir /s /q `"$Root`""
    Write-Host 'Removed. buysg will be gone in new terminals.'
    exit 0
}

Push-Location $env:BUYSG_HOME
try {
    & $Exe @args
    exit $LASTEXITCODE
}
...
'@
    [IO.File]::WriteAllText($PsShimPath, $shimPs,
        (New-Object System.Text.UTF8Encoding($false)))
}
...
if (Test-Path $PsShimPath) { & $PsShimPath doctor }
else { & (Join-Path $Bin 'buysg.cmd') doctor }
