...
4. 建立獨立執行環境與 `stock-gift` 指令（加入使用者 PATH，不需系統管理員）
5. 執行 `stock-gift doctor` 環境健檢

> 安裝當下的視窗可直接使用 `stock-gift`；其他已開啟的視窗需重開才會認得指令。
...
```
%LOCALAPPDATA%\stock-gift\home\certificates\
└── <esun 或 fubon>\
...
- 放好後執行 `stock-gift doctor`，會逐項告訴你還缺什麼
- 帳戶資料夾會**自動掃描**，執行時在選單勾選要用哪些帳戶

## 設定檔（config.ini）

第一次執行 `stock-gift` 會在 `%LOCALAPPDATA%\stock-gift\home\config\config.ini`
**自動生成**安全預設：
...
| 指令 | 說明 |
|---|---|
| `stock-gift` | 互動選單（分類 / 價值 / 股價上限 / 帳戶勾選） |
| `stock-gift doctor` | 環境健檢（設定 / 憑證 / SDK / 資料連線） |
| `stock-gift update` | 更新到最新版 |
| `stock-gift uninstall` | 完整移除（會提醒先備份憑證） |

## 資料夾結構

```
%LOCALAPPDATA%\stock-gift\
...
- 任何問題先跑 `stock-gift doctor`，它會逐項標出缺什麼、該放哪
- 新開的視窗找不到 `stock-gift` 指令 → 重開一個 PowerShell 視窗
- 想砍掉重來：`stock-gift uninstall` 後重跑安裝一行（憑證記得先備份）
